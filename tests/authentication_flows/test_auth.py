...
class OAuth2AuthFlows(TestCase):
    ...
    def test_client_credentials_flow(self):
        ...
        auth_headers = base64.b64encode(
            f"{settings.TEST_CLIENT_ID_CLIENT_CRED}:{settings.TEST_CLIENT_SECRET_CLIENT_CRED}".encode(
                "utf-8"
            )  # noqa:E501
        ).decode("utf-8")
    ...
    def test_resource_owner_password_flow(self):
        ...
        auth_headers = base64.b64encode(
            f"{settings.TEST_CLIENT_ID_OWNER}:{settings.TEST_CLIENT_SECRET_OWNER}".encode(
                "utf-8"
            )  # noqa:E501
        ).decode("utf-8")
    ...
    def test_userinfo_endpoint(self):
        ...
        response = self.client.get(
            userinfo_url, HTTP_AUTHORIZATION=f"Bearer {access_token}"
        )
